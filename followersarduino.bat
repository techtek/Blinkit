...
:: Let the user know that led 2 is going to be blinked, on the arduino, and display the com and baudrate:  	  
	echo %Magenta%Arduino%White% %com% baudrate %baud%
	echo.

:: Test Blink the LED, by sending the f command to the Arduino
	echo %Magenta%Testing %White%Blink LED 
	echo f> config/arduinocommand.txt
...
:: Let the user know, there is a new Follower, and blink the Followers led
	echo f> config/arduinocommand.txt
	start arduinosendcommand.exe  
