...
:: Let the user know that Blinkit is going to watch for Mentions by displaying the text:
	echo Blinkit - Steem Account Mentions
...
:: Display the saved Username and Flash drive letter and let the user know that the program is starting to look for new Mentions
	echo.
...
:: Download new data to compare, into "download2.txt", and go back to "main" to compare the files again
	powershell -Command "Invoke-WebRequest http://api.comprendre-steem.fr/getMentions?username=%steemusername% -OutFile C:\blinkit\config\downloadmentions2.txt" 
	goto main

	
:notification
:: Let the user know, there is a new Mention! by displaying the text:  
	echo Blinkit is running... %Blue%NEW MENTION for %steemusername%!
	echo %White%
...
:: Download new data to compare, and go back to "main" and continue to look for for new Mentions.
	powershell -Command "Invoke-WebRequest http://api.comprendre-steem.fr/getMentions?username=%steemusername% -OutFile C:\blinkit\config\downloadmentions2.txt"	
	goto main
