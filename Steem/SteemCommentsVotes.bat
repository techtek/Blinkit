...
:set   
    ...
    set jsonbody='{\"jsonrpc\":\"2.0\", \"method\":\"condenser_api.get_blog\", \"params\":[\"%steemusername%\",0,2], \"id\":1}' 
...
:menu   
    cls
	echo.
	echo %Grey%Blinkit USB Flash Notifications %Blue%Steem %Grey%Account post votes%White% 
...
:: Let the user know that Blinkit is going to watch for Upvotes by displaying the text:
	echo %Blue%Steem account comments and votes %Grey%(limit is latest 2 blog posts)
	echo.
...
:: Download postvotes data from Steem API and save it into a txt file
	 powershell Invoke-RestMethod -ContentType 'application/json' -Method Post -Uri https://api.steemit.com -Body %jsonbody% -UserAgent "curl" -OutFile  %~dp0\data\downloadedpostvotes.txt"
	PING localhost -n 4 >NUL
	

:main   
:: Download postvotes data from Steem API and save it into a txt file
	 powershell Invoke-RestMethod -ContentType 'application/json' -Method Post -Uri https://api.steemit.com -Body %jsonbody% -UserAgent "curl" -OutFile  %~dp0\data\downloadedpostvotes2.txt"
	 PING localhost -n 4 >NUL
...
:: Let the user know, there is a new comment/votes by displaying the text:   
	echo.
	echo %White%Blinkit new comment/vote for %Blue%Steem %White%user %steemusername% detected, %Green%Light blink! %White% 
